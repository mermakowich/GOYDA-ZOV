...
editElementHandler = function (event) {
    const target = event.target;
    const tagName = target.tagName;
    console.log(tagName);
    if (tagName === "IMG") {
        let currentSrc = target.src;
        let src = prompt("Вы хотите поменять значение?", currentSrc);
        if (src) {
            target.src = src;
        }
    } else {
        let currentText = target.textContent;
        let text = prompt("Вы хотите поменять значение?", currentText);
        if (text) {
...
const addElementHandler = function (event) {
    ...
    elementClone.querySelector(".template-content")
    .addEventListener("dblclick", editElementHandler);
